...
这是一个「CLI → 本地管理服务 → 浏览器工作台」形态构建的真实本地软件项目。它不再依赖单个 HTML 文件保存核心数据：项目、需求、发布计划和设置持久化到 SQLite，管理页由本地 Node.js 服务提供。当前版本 v0.9.6，完整变更记录见 [CHANGELOG.md](./CHANGELOG.md)。

## 核心能力

多项目需求管理、列表/看板/发布计划视图、批量操作、评审模式、周报、CSV/JSON 导入导出、Jira 五段式复制和 AI Jira 标准化往返；活态批注讨论支持按需求区域记录产品、设计、研发、测试和业务意见，并支持解决或重新打开；AI 需求评审采用不保存 API Key 的复制往返方式，评审结果可存档并标记采纳、驳回或解决；需求级版本历史可以保存操作者、变更原因和字段变化，并比较相邻版本差异。

v0.9.6 是需求池易用性升级版：收拢核心统计、搜索、筛选、列表字段和行内操作，增加列表密度与显示列偏好；批量操作统一返回成功与跳过数量，标准文档批量录入支持分级校验、阻断项保留和结果回看，评审与流程阶段继续共用同一状态口径。
...
```bash
npm run check
node dist/cli.js start --host 127.0.0.1 --port 8080
```

重点验收：本机访问应保留本地身份；局域网地址访问应按游客处理；需求列表筛选、批量确认、发布计划详情、AI 提案、团队同步预览和快照恢复均应可正常打开。
...
通过局域网地址访问的用户会被自动识别为“局域网游客”，默认只读：可以查看项目、需求、发布计划和导出类内容，但不能进入工作台、系统设置、AI 工作台、导入恢复、主动同步、新建、编辑或批量维护流程。请只在可信局域网使用，并配合系统防火墙限制访问范围。
